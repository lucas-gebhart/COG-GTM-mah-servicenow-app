...
| Workspace route ACL (`ux_route`, `x.cog.mah-operations.*`) | Fluent `Acl()`, **generated** | same file |
...
* Title `MAH Operations`, URL `/x/cog/mah-operations` (scoped experiences are served under
  `/x/<vendor>/<path>`, not `/now/<path>`), landing route `home`.
...
* Route ACL `x.cog.mah-operations.*` (type `ux_route`, operation `read`; the URL with slashes
  replaced by dots, as in the platform's own `x.snc.notification-dashboard.*`) grants the workspace to
...
the workspace by a `par_dashboard_visibility` record, which is why `x/cog/mah-operations/home`
...
| Module | Link | Roles |
|---|---|---|
| Operations dashboard | `x/cog/mah-operations/home` | awards-case readers (tacom_staff, csr, engraver, assembler, warehouse, dla, admin) |
...
`x.cog.mah-operations.*` `sys_security_acl`.
...
1. Application navigator > *MAH Case Management*: a `MAH Operations` separator followed by the
   ...
   filtered list, and *Operations dashboard* opens `/x/cog/mah-operations/home`.
...
| Role | Expected |
|---|---|
| `x_cog_mah.vendor` | only the `Vendor work` module; no access to `/x/cog/mah-operations` |
