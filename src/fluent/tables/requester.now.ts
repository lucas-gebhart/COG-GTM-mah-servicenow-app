...
export const x_cog_mah_requester = Table({
    ...
    schema: {
        ...
        legacy_number: StringColumn({ label: 'Legacy requester key', maxLength: 40 }),
        ...
        rank: StringColumn({ label: 'Rank / grade', maxLength: 10 }),
        role_title: StringColumn({ label: 'Duty position', maxLength: 60 }),
        dodaac: StringColumn({ label: 'DODAAC', maxLength: LIMITS.dodaac }),
        uic: StringColumn({ label: 'UIC', maxLength: LIMITS.uic }),
        veteran_name: StringColumn({ label: 'Veteran name (for next of kin)', maxLength: 160 }),
        preferred_contact: StringColumn({ label: 'Preferred contact', maxLength: 20 }),
    ...
    index: [
        ...
        { name: 'idx_req_legacy_number', unique: false, element: 'legacy_number' },
    ],
