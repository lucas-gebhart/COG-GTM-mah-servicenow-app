...
export const x_cog_mah_ses_flag_request = Table({
    ...
    schema: {
        ...
        legacy_number: StringColumn({ label: 'Legacy SES flag number', maxLength: 40 }),
        requesting_office: StringColumn({ label: 'Requesting office', maxLength: LIMITS.name, mandatory: true }),
        executive_tier: StringColumn({ label: 'Executive tier', maxLength: 10 }),
        dodaac: StringColumn({ label: 'DODAAC', maxLength: LIMITS.dodaac }),
        uic: StringColumn({ label: 'UIC', maxLength: LIMITS.uic }),
        legacy_flag_type: StringColumn({ label: 'Legacy flag type', maxLength: 60 }),
        vendor: ReferenceColumn({ label: 'Vendor', referenceTable: 'x_cog_mah_vendor', cascadeRule: 'clear' }),
